function mudou_a_UF(x) {
  // Lista de cidades organizadas por UF
  ...
  let quais_opt_de_cidade = [];
  const estado_escolhido = x.value;

  // Filtra as cidades para o estado selecionado
  cidades_por_estados.forEach((item) => {
    if (item[0] === estado_escolhido) {
      quais_opt_de_cidade.push(item[1]);
    }
  });

  // Atualiza o select de cidade
  const citySelect = document.getElementById("input-city");
  citySelect.innerHTML = '<option value="">Selecione a Cidade</option>'; // reinicia as opções

  quais_opt_de_cidade.forEach((cidade) => {
  ...
  console.log("Cidades disponíveis para " + estado_escolhido + ":", quais_opt_de_cidade);
}